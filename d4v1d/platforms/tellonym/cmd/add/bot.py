"""
Create a new bot for the tellonym
...
class AddBot(Command):
    def __init__(self) -> None:
        ...
        """
        super().__init__('add bot', description='Create a new bot for the `tellonym` platform.')
    ...
    def execute(self, raw_args: List[str], argv: List[str], state: CLISessionState, *args,
        ...
        if not group_name:
            ...
            return
        if not user_agent:
            user_agent = random.choice(config.PCONFIG._tellonym.user_agents)
        ...
        if group_name not in state.platform.groups:
            ...
            return
        group: TellonymGroup = state.platform.groups[group_name]
        bot: TellonymBot = TellonymBot(nickname, group, anonymous=anonymous, user_agent=user_agent,
